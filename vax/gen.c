...
/*  The VAX has 12 integer registers r0-r11.
 *  It has no separate floating registers.
 *  Doubles occupy an even-odd pair of ordinary regs.
 *  Function call results are returned in r0 and r1 for doubles.
 *  r0 through r5 are presumed destroyed by callee and must be saved
 *  by the caller.
...
void function(Symbol f, Symbol caller[],
	...
	offset = 4;		/* offset of leftmost parameter */
...
void emit(register struct node *ins)
	...
	case ASGNB:
	    save(ins->x.busy & 0x3f);
	    print("movc3 $%s,(r%d),(r%d)\n", ins->syms[0]->x.name,
		b->x.reg, a->x.reg);
	    restore(ins->x.busy & 0x3f);
	    break;
