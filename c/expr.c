...
Tree asgn(p, e) Symbol p; Tree e; {
	...
	if (isarray(p->type))
		...
		if (isstruct(p->type) && p->type->sym->u.s.cfields) {
			p->type->sym->u.s.cfields = 0;
			e = asgnnode(ASGN, idnode(p), e, 0);
			p->type->sym->u.s.cfields = 1;
		} else
			e = asgnnode(ASGN, idnode(p), e, 0);
		p->type = ty;
	}
...
Type assign(xty, p) Type xty; Tree p; {
	...
	if (isptr(xty) && isint(p->type)) {
		p = rightkid(p);
		if (p->op == CNST+I && p->u.v.i == 0)
			return xty;
	}
	return 0;
}
...
static Tree call(f, fty, src) Tree f; Type fty; Coordinate src; {
	int n = 0;
	Tree args = 0, r = 0;
	Symbol proto = fty->sym;

	if (proto && proto->type == voidtype)
		proto = 0;
	if (hasop(f, CALL))
		r = f;
	if (t != ')')
		do {
			Tree q = pointer(expr1(0, 0));
			if (proto && proto->type != voidtype) {
				Type aty;
				q = value(q);
				if (aty = assign(proto->type, q)) {
					q = cast(q, aty);
					...
					error("type error in argument %d to %s; found `%t' expected `%t'\n",
						n + 1, funcname(f), q->type, proto->type);
				proto = proto->u.proto;
			} else {
				if (fty->sym && proto == 0)
	...
	if (YYlink)
		f = tree(RIGHT, f->type, bbcall(src), f);
...
Tree expr1(tok, many) {
	...
	|| (prec[t] >= 11 && prec[t] <= 13)) {
		...
		t = gettok();
		if (oper[op] == ASGN)
			p = asgnnode(ASGN, p, value(expr1(0, many + 1)), many);
		else {
			expect('=');
			p = incr(op, p, expr1(0, many + 1), many);
		}
	many = 1;
	}
	if (tok && t == tok)
...
Tree incr(op, e1, e2, protect) Tree e1, e2; {
	return asgnnode(ASGN, e1, (*opnode[op])(oper[op], e1, e2), protect);
}
...
int intexpr(tok, n) {
	...
	if (generic(p->op) == CNST && isint(p->type)) {
		if (isunsigned(p->type))
			return p->u.v.u;
		return cast(p, inttype)->u.v.i;
	}
	error("integer expression must be constant\n");
	return n;
}

/*  load -- force the expression to appear in a register */
Tree load(p) Tree p; {
	if (generic(p->op) == LOAD || generic(p->op) == CNST ||
		generic(p->op) == ASGN)
	    return p;
	return tree(LOAD + ttob(p->type), p->type, p, 0);
}
...
static Tree nullcheck(p) Tree p; {
	Tree q, arg = tree(ARG+I, inttype, constnode(lineno, inttype), 0);

	q = tree(COND, voidtype,
		tree(NOT, inttype, cond(p), 0),
		tree(RIGHT, inttype, callnode(idnode(YYnull), voidtype, arg), 0));
...
static Tree postfix(p) Tree p; {
	...
	for (;;)
		...
		case INCR: case DECR:
			...
			p = tree(RIGHT, p->type,
				tree(RIGHT, p->type, q,
				    root(incr(t, p, constnode(1, inttype), 0))),
				q);
			t = gettok();
			continue;
		case '[':
...
/* include unaries */
	...
	case '!':
		...
		break;
/* end unaries */
/* include preincrement */
	case INCR: case DECR: {
		Opcode op = t;
		t = gettok();
		p = incr(op, pointer(prefix()), constnode(1, inttype), 1);
		break;
		}
/* end preincrement */
/* include sizeof */
