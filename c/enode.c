...
/* asgnnode - construct tree for l = r.  Mark r if protect nonzero */
Tree asgnnode(op, l, r, protect) Tree l, r; {
	Type aty, ty;
	...
	if (isptr(aty))
		aty = unqual(aty)->type;
	if (isconst(aty) || (isstruct(aty) && unqual(aty)->sym->u.s.cfields))
	...
	return tree(op + (isunsigned(ty) ? I : ttob(ty)), ty, l,
		(protect) ? load(r) : r);
}
...
/* compatible - are ty1 & ty2 sans qualifiers compatible pointer types? */
...
Tree condnode(e, l, r) Tree e, l, r; {
	...
	if (isarith(l->type) && isarith(r->type)) {
		...
		l = cast(l, ty);
		r = cast(r, ty);
	} else if (eqtype(l->type, r->type)
	|| isptr(l->type) && isint(r->type) && assign(l->type, r))
		ty = unqual(l->type);
	else if (isptr(r->type) && isint(l->type) && assign(r->type, l))
		ty = unqual(r->type);
	else if (isptr(l->type) && isptr(r->type)) {
		if (unqual(l->type->type) == voidtype && l->op == CNST+P && l->u.v.p == 0)
			ty = unqual(r->type->type);
		else if (unqual(r->type->type) == voidtype && r->op == CNST+P && r->u.v.p == 0)
			ty = unqual(l->type->type);
		else if (unqual(l->type->type) == voidtype
		||       unqual(r->type->type) == voidtype)
			ty = voidtype;
		else if (compatible(l->type, r->type))
			ty = unqual(l->type->type);
		if (ty) {
			if (isconst(l->type->type) || isconst(r->type->type))
				ty = qual(CONST, ty);
			if (isvolatile(l->type->type) || isvolatile(r->type->type))
				ty = qual(VOLATILE, ty);
			ty = ptr(ty);
		}
	}
