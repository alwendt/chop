...
static struct dag *dagnode(op, l, r, sym) Node l, r; Symbol sym; {
	...
	BZERO(p, struct dag);
#if SABER
	saber_untype(p, sizeof(struct dag));
#endif
	p->node.op = op;
	if (p->node.kids[0] = l)
		++l->count;
...
void emitcode() {
	...
	for (bp = 0, cp = &codehead; errcnt <= 0 && cp; cp = cp->next)
		...
		case Blockend:
			if (glevel) {
				foreach(typestab, bp->u.block.identifiers, bp->u.block.level);
				foreach(typestab, bp->u.block.types, bp->u.block.level);
				stabblock('}', bp->u.block.level - LOCAL, bp->u.block.locals);
				swtoseg(CODE);
			}
			bp = bp->u.block.prev;
...
void gencode(caller, callee) Symbol caller[], callee[]; {
	...
	for (i = 0; (p = callee[i]) && (q = caller[i]); i++)
		if (p->class != q->class || p->type != q->type) {
...
Node listnodes(tp, tlab, flab) Tree tp; {
	...
	case RIGHT:
		if (tp->kids[0] && generic(tp->kids[0]->op) == LOAD /* INDIR */
		&&  tp->kids[1] && generic(tp->kids[1]->op) == ASGN
		&&  tp->kids[0]->kids[0]->kids[0] == tp->kids[1]->kids[0]) {
			/* e++ */
	...
	case RSH:
		...
		l = listnodes(tp->kids[0], 0, 0);
/*
		Die, crufty undocumented vax-dependency.
		#ifdef VAX
		if (tp->op == RSH+I)
...
*/
	...
	case CVP: case CVS: case CVU: case NEG: case BCOM: case LOAD:
		assert(tlab == 0 && flab == 0);
		l = listnodes(tp->kids[0], 0, 0);
		p = node(tp->op, l, 0, 0);
		break;
	case INDIR: {
		...
		assert(tlab == 0 && flab == 0);
		l = listnodes(tp->kids[0], 0, 0);
		if (isvolatile(ty) || (isstruct(ty) && unqual(ty)->sym->u.s.vfields))
			p = newnode(tp->op, l, 0, 0);
		else
			p = node(tp->op, l, 0, 0);
		break;
...
static void typestab(p) Symbol p; {
	if (!isfunc(p->type) && (p->class == EXTERN || p->class == STATIC))
		stabsym(p);
	else if (p->class == TYPEDEF || p->class == 0)
		stabtype(p);
}
...
static Node undag1(p, root) Node p, root; {
	...
	} else if ((p->op == ADDRL+P || p->op == ADDRF+P) && p->syms[0]->class == REGISTER) {
		assert(p != root);
		p = newnode(p->op, 0, 0, p->syms[0]);
		p->count = 1;
	} else if (generic(p->op) == INDIR
	&& (p->kids[0]->op == ADDRL+P || p->kids[0]->op == ADDRF+P)
	&& p->kids[0]->syms[0]->class == REGISTER && p != root) {
		p = newnode(p->op, 
