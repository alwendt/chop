...
static void caselabel(swp, val, lab) struct swtch *swp; {
	...
	++swp->ncases;
	if (Aflag >= 2 && swp->ncases == 128)
		warning("more than 127 cases in a switch statement\n");
}
...
Code definept(p) Coordinate *p; {
	...
	if (YYcounts)
		listnodes(bbincr(cp->u.point.src), 0, 0);
	if (glevel > 2)
...
static void forstmt(lab, swp, lev) struct swtch *swp; {
	...
	if (kind[t] == ID)
		...
		test(')', follow);
	if (glevel > 1) {	/* e1; L: if (!e2) goto L+2; S; L+1: e3; goto L; L+2: */
		definelab(lab);
		definept(&pt2);
		if (e2)
			walk(e2, 0, lab + 2);
...
/*  This function compares the type of the expression in p with the type of the
 *  function and inserts any necessary conversions.  It then inserts a RETx code.
 *  If BRANCH_ON_RETURN is defined, the lab field will be nonzero, and the RETx
 *  code means to "load the expression into the function result register".  
 *  Otherwise, RETx means "load the expression into the function's result register
...
void retcode(p, lab) Tree p; {
	if (p == 0) {
		...
		if (YYprintf) {
			Symbol x = genident(AUTO, p->type, 0);
			addlocal(x);
			walk(root(asgn(x, p)), 0, 0);
			tracereturn(cfunc, x);
			p = idnode(x);
		}
		...
		if (retv) {
			...
			&& p->kids[1]->op == INDIR+B) {
				...
				p = asgnnode(ASGN, rvalue(idnode(retv)), p, 0);
			p = root(p);
			if (YYlink)
				p = tree(RIGHT, voidtype, p, bbexit());
			p = tree(RIGHT, voidtype, p, tree(RET+V, voidtype, 0, 0));
		} else {
			if (isptr(p->type)) {
				Symbol q = localaddr(p);
				if (q && (q->computed || q->generated))
					error("pointer to a %s is an illegal return value\n",
						q->scope == PARAM ? "parameter" : "local");
				else if (q)
					error("pointer to %s `%s' is an illegal return value\n",
	...
	if (YYlink && generic(p->op) == RET)
		p = tree(RIGHT, voidtype, bbexit(), p);
	walk(p, 0, 0);
	branch(lab);
}
...
static void swstmt(loop, lab, lev) {
	...
	&& e->kids[0]->u.sym->type == e->type && !isvolatile(e->kids[0]->u.sym->type)) {
		...
		sw.sym = genident(REGISTER, e->type, 0);
		addlocal(sw.sym);
