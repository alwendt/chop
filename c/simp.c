...
dclproto(static int ispow2,(unsigned u))
...
static int ispow2(u) unsigned u; {
	int n;
...
/*  put the lower-numbered opcode on the left */
#define commute(L,R) \
	if ((R->op < L->op) || \
	    (R->op == L->op && generic(R->op) == INDIR && R->kids[0]->op < L->kids[0]->op)) \
	    { Tree t = L; L = R; R = t; }
...
Tree simplify(op, ty, l, r) Type ty; Tree l, r; {
	...
	case GE+D: cfoldcnst(D,d,>=,inttype);
