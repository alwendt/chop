...
#ifdef sun
		} else if (*argv[i] == '-' && strcmp(argv[i], "-target") == 0) {
...
static void opt(arg) char *arg; {
	...
	case 'W':
		if (arg[2] && arg[3])
			switch (arg[2]) {
			case 'o': if (option(&arg[3])) return; break;
	...
	case 'd':
		arg[1] = 's';
		/* fall thru */
	case 'A': case 'n': case 'w': case 'P': case 't':
	...
	case 'c':
		cflag++;
		break;
	case 'D': case 'I': case 'U': case 'C': case 'T':
