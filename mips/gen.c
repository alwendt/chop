...
#else CHOPEMIT
static RegSet rmask;	/* rmask&(1<<r) == 0 if register r is free */
static RegSet usedmask;	/* usedmask&(1<<r) == 1 if register r was used */
...
static int spillee(Node, RegSet);
static void spill(int, RegSet, Node);
#endif CHOPEMIT
...
void function(Symbol f, Symbol caller[],
	 ...
	 *  so that the front end will emit code to store them into the frame.
	 */
...
static void getreg(Node p) {
	...
	for (r = 0; r < nregs; r++)
		if ((rmask& (m<<r)) == 0) {
			...
			rmask = OR(rmask, sets(p));
			usedmask = OR(usedmask, sets(p));
...
static void spill(int r, RegSet m, Node dot) {
	int i;
	Node p = dot;
	int first, lim;
	...
	while (p = p->x.next) {
		...
		for (i = first; i < lim; i++)
			if (IKID(p,i) && sets(IKID(p,i))&(m<<r)) {
				...
				rmask = AND(rmask, NOT(sets(IKID(p,i))));
...
static int spillee(Node dot, RegSet m) {
